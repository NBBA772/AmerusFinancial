...
import { isRef as isRef2, ref as ref2, shallowRef as shallowRef2, watchEffect as watchEffect2, computed as computed2, inject as inject2, unref as unref2, watch as watch2, getCurrentInstance as getCurrentInstance2, customRef as customRef2, onUpdated, reactive as reactive2, nextTick as nextTick2, onMounted as onMounted2, markRaw, readonly as readonly2, getCurrentScope as getCurrentScope2, isVue2 as isVue22, set as set2, del, isReadonly, onBeforeUpdate } from "vue-demi";
...
function useCloned(source, options = {}) {
  ...
  if (!manual && isRef2(source)) {
    watch2(source, sync, __spreadProps$8(__spreadValues$l({}, options), {
      deep,
      immediate
    }));
  } else {
...
function useDraggable(target, options = {}) {
  ...
  return __spreadProps$42(__spreadValues$f({}, toRefs(position)), {
    position,
...
function useFileSystemAccess(options = {}) {
  ...
  async function open(_options = {}) {
    if (!isSupported.value)
      return;
    const [handle] = await window2.showOpenFilePicker(__spreadValues$b(__spreadValues$b({}, unref2(options)), _options));
  ...
  async function create(_options = {}) {
    if (!isSupported.value)
      return;
    fileHandle.value = await window2.showSaveFilePicker(__spreadValues$b(__spreadValues$b({}, unref2(options)), _options));
    data.value = void 0;
    await updateFile();
    await updateData();
  }
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  const state = reactive2(useScroll(element, __spreadProps$22(__spreadValues$92({}, options), {
...
function useClamp(value, min, max) {
  if (isFunction(value) || isReadonly(value))
    return computed2(() => clamp(resolveUnref(value), resolveUnref(min), resolveUnref(max)));
...
function usePointer(options = {}) {
  ...
  return __spreadProps$12(__spreadValues$62({}, toRefs(state)), {
    isInside
  });
}
...
function usePrevious(value, initialValue) {
  ...
  watch2(resolveRef(value), (_, oldValue) => {
...
function useTimeAgo(time, options = {}) {
  ...
  const timeAgo = computed2(() => formatTimeAgo(new Date(resolveUnref(time)), options, unref2(now2.value)));
  if (exposeControls) {
